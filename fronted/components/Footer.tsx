  ...
  return (
    <footer className="bg-[#1A1A1A] text-white pt-24 pb-12 px-6 md:px-12">
      <div className="container mx-auto grid grid-cols-1 md:grid-cols-3 gap-20">
        {/* Left: Brand */}
        <div>
          <h2 className="text-2xl heading-font font-bold tracking-[0.3em] mb-8 cursor-pointer" onClick={() => onNavigate('home')}>U N I Q U E</h2>
        ...
        <div className="grid grid-cols-2 gap-8">
          <div className="space-y-4">
            ...
            <ul className="text-gray-400 text-[10px] space-y-3 uppercase tracking-widest">
              <li><button onClick={() => onNavigate('policy')} className="hover:text-white transition-colors">配送与退换 Policies</button></li>
              <li><button onClick={() => onNavigate('faq')} className="hover:text-white transition-colors">常见问题 FAQ</button></li>
            </ul>
          </div>
          <div className="space-y-4">
            <h3 className="text-xs uppercase tracking-widest mb-6">发现 Explore</h3>
            <ul className="text-gray-400 text-[10px] space-y-3 uppercase tracking-widest">
              <li><button onClick={() => onNavigate('story')} className="hover:text-white transition-colors">品牌故事 Story</button></li>
              <li><button onClick={() => onNavigate('artists')} className="hover:text-white transition-colors">创作者名录 Artists</button></li>
              <li><button onClick={() => onNavigate('bespoke')} className="hover:text-white transition-colors">定制服务 Bespoke</button></li>
            </ul>
          </div>
        </div>
