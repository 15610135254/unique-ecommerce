...
import { useNewProducts } from '../src/hooks/useProducts';
...
const NewArrivalsPage: React.FC<NewArrivalsPageProps> = ({ onSelectProduct }) => {
  ...
  if (products.length === 0) {
    return (
      <div className="flex justify-center items-center min-h-[60vh]">
        <p className="text-xs text-gray-400">暂无新品</p>
      </div>
    );
  }
    ...
    <div className="animate-fade-in pb-24">
      ...
      <section className="h-[60vh] flex flex-col md:flex-row mb-24">
        ...
        <div className="w-full md:w-1/2 h-full">
          <img src={products[0]?.image || products[0]?.imageUrl} className="w-full h-full object-cover grayscale" />
        </div>
      </section>
          ...
          {products.slice(1).map((product, idx) => (
            ...
            >
              <div className="relative overflow-hidden mb-8">
                <img src={product.image || product.imageUrl} className="w-full aspect-[4/5] object-cover transition-transform duration-1000 group-hover:scale-105" />
